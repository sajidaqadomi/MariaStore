...
import { AUTH, AUTH_END_LOADING, AUTH_ERROR, AUTH_START_LOADING, SIGN_OUT } from '../utility/actionTypes'

export const signUp = (user, navigate) => async (dispatch) => {
    try {
        dispatch({ type: AUTH_START_LOADING })
        const { data: { _id } } = await api.signUp(user)
...
export const signIn = (user, navigate) => async (dispatch) => {
    try {
        ...
        if (data.userToken) {
            ...
            dispatch({ type: AUTH, payload: { id: user.id, isAdmi: user.isAdmin, userName: user.userName } })
            navigate('/')
